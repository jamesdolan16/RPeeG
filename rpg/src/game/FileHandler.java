...
public class FileHandler {
	...
	public static ArrayList<String> LoadAreaList() {
		...
		try {
			...
			ScreenDrawer.printError("Unable to open "+areaListPath+" for reading.\n");
			return null;
		}
	...
	public static Area LoadElementList(String listName, Area pLoaded, JSONObject jsonObj) {
		Area toRet = pLoaded;
		...
		try {
			...
			while(itr.hasNext()) {
				...
				switch(listName) {
				case "doors":
					toRet.Doors.add(new Door(id, pos));
					break;
				case "items":
					toRet.Items.add(new Item(id, pos));
					break;
				case "actors":
					toRet.Actors.add(new Actor(id, pos));
				}
			}
			return toRet;
		}catch(Exception e) {
			ScreenDrawer.printError("Failed to load element list "+listName+"\n"+e);
			return pLoaded;
		}
	}
	...
	public static Area LoadArea(String areaID) {
		...
		try {
			...
			ScreenDrawer.printError("Unable to open area file "+path+" for reading.\n");
			return null;
		}
		JSONObject jsonObj = (JSONObject) parseObj;
		...
		toRet = LoadElementList("doors", toRet, jsonObj);
		toRet = LoadElementList("items", toRet, jsonObj);
		toRet = LoadElementList("actors", toRet, jsonObj);
	...
	public static Door LoadDoor(String doorID) {
		...
		try {
			...
			ScreenDrawer.printError("Unable to open door file "+path+" for reading.\n");
			return null;
		}
		JSONObject jsonObj = (JSONObject) parseObj;
	...
	public static Item LoadItem(String itemID) {
		...
		try {
			...
			ScreenDrawer.printError("Unable to open item file "+path+" for reading.\n");
			return null;
		}
		JSONObject jsonObj = (JSONObject) parseObj;
